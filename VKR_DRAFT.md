...
REST API реализован на DRF и формирует единый контракт для мобильных/внешних клиентов и внутренних административных операций. Слой API включает:
- клиентские ресурсы (`cars`, `appointments`, `service-types`, `service-centers`, `profile`);
- справочные ресурсы (`car-brands`, `car-models`);
- административные ресурсы с префиксом `admin-panel/*` (записи, статистика, отзывы, слоты, клиенты, услуги, филиалы, сотрудники, дашборд);
- endpoints аутентификации и регистрации (JWT + регистрация).

Маршрутизация API построена через `DefaultRouter`, что упрощает масштабирование набора эндпоинтов и поддержку единообразной структуры URL. Ниже приведен сокращенный фрагмент регистраций:
...
Практический эффект такого подхода:
- унификация доступа к бизнес-функциям для разных клиентов;
- явное разграничение клиентского и административного API-контуров;
- предсказуемое расширение системы за счет добавления новых ViewSet без изменения базовой схемы маршрутизации и аутентификации.

#### 1.5.2 Компоненты веб-клиента

Веб-клиент реализован как server-side рендеринг на Django templates и покрывает два контура: пользовательский и административный. Пользовательский контур включает регистрацию, профиль, управление автомобилями, запись и просмотр деталей обслуживания. Административный контур содержит дашборд, управление филиалами, услугами, клиентами, записями, отзывами и блокировкой временных слотов.
...
Выбранный подход обеспечивает низкий порог сопровождения, быстрый вывод функционала в эксплуатацию и согласованность поведения с REST API, так как оба интерфейса используют общий backend-слой бизнес-правил.
