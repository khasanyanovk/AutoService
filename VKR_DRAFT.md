...
Базовая команда верификации проекта — `python manage.py test`. Для ускорения в CI используется запуск с `--keepdb`, что сокращает время повторных прогонов при сохранении полноты проверок; при изменениях схемы БД предпочтителен запуск без `--keepdb` для исключения влияния устаревшего тестового состояния.
