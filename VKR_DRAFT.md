...
Клиентская часть представлена двумя самостоятельными контурами: пользовательский веб-интерфейс (`core`) и административный интерфейс (`admin_panel`). В этой главе я рассматриваю их как один прикладной слой, который работает поверх единой серверной модели данных и не дублирует бизнес-правила.

По объему реализации клиентская часть включает 60 view-функций (25 в `core`, 35 в `admin_panel`), 18 серверных форм (9 + 9) и 54 HTML-шаблона (29 + 25). Такой объём покрывает полный цикл ключевых сценариев: регистрация, управление автомобилями, запись на обслуживание, оперативная обработка записей в админ-панели, работа с отзывами и расписанием.

На практике это дало предсказуемые пользовательские потоки: клиент видит только собственные данные и действия, администратор работает с операционными сценариями, а итоговое поведение определяется серверной валидацией и статусной логикой.

#### 2.1.3 Маршрутизация и разделение контуров

Для разделения ответственности я использую явную маршрутизацию на уровне проекта: пользовательские маршруты, платежный модуль, административный модуль и REST API подключаются отдельно.
...
Такой подход упрощает сопровождение: изменения в одном контуре маршрутов не затрагивают остальные, а доступ к административным операциям остаётся изолированным.

#### 2.1.4 API-слой как часть общей архитектуры

REST API подключен через `DefaultRouter`, что дает единый формат endpoint-структуры и позволяет расширять API без ручного дублирования шаблонной маршрутизации.
...
За счет этого API и веб-часть остаются согласованными по доменной модели и правам доступа.

### 2.2 Реализация серверной части (Django backend)

В этой части я подробно фиксирую, как бизнес-правила реализованы на backend-уровне: от схемы данных и валидации до обработки статусов, интеграций и ошибок. Цель раздела — показать не только набор функций, но и то, как они работают как единая система.

Серверная реализация построена на нескольких опорных принципах: единый источник правил на стороне backend, прозрачные статусные переходы, отказ от «доверия» к клиентскому вводу, а также явная изоляция административных сценариев. За счет этого прикладная логика остается стабильной не только в стандартном сценарии, но и в пограничных случаях (конфликтные слоты, повторные запросы, неполные данные, несогласованные действия в интерфейсе).

На практике backend выступает управляющим слоем предметной области: именно здесь закреплены правила расписания, ролевые ограничения, условия изменения статусов, интеграционные проверки платежей и контроль консистентности записей.
...
Такой подход позволяет формализовать бизнес-ограничения в явных, проверяемых правилах. За счёт этого критичные проверки не зависят от клиентского JavaScript и выполняются единообразно для всех точек входа.

Дополнительно в этом же слое реализована валидация зависимых справочников для автомобилей:
...
Ещё один практический пример — валидация регистрационных данных автомобиля:

```python
pattern = r"^[А-Я]{1}\d{3}[А-Я]{2}$|^[A-Z]{1}\d{3}[A-Z]{2}$"
if not re.match(pattern, plate):
    raise ValidationError("Номер должен быть в формате X000XX.")
```

Такое построение валидации снижает стоимость сопровождения: локальные изменения интерфейса не требуют переписывать доменные ограничения, а регрессии проще выявляются тестами.

#### 2.2.2 Пользовательские Django views и обработка бизнес-сценариев

Пользовательские сценарии реализованы через function-based views в `core/views.py` с обязательной авторизацией там, где есть доступ к персональным данным и записям. Внутри представлений я использую `select_related/prefetch_related`, ветвления GET/POST, возврат HTML/JSON для асинхронных элементов интерфейса и единый формат пользовательских сообщений.

Код не ограничивается базовым CRUD. Например, в профиле формируются аналитические агрегаты, а в booking-потоке запрос проходит через цепочку серверных проверок перед сохранением:
...
Для динамического UI используется JSON-ответ с готовыми слотами:
...
Отдельно решена задача «мягкой» деградации: вместо необработанных исключений пользователь получает понятное сообщение и корректный следующий шаг.

#### 2.2.3 Административный backend: контроль процессов и операционная аналитика

Административные операции реализованы в `admin_panel/views.py` и изолированы декораторами `@login_required` и `@admin_required`. Этот слой покрывает ежедневные операционные задачи: фильтрацию, поиск, пагинацию, обновление статусов, работу с отзывами и формирование KPI для дашборда.
...
Похожий принцип используется в админ-поиске и сегментации пользовательской базы:
...
Сервер автоматически поддерживает корректность жизненного цикла записи и снимает часть ручной нагрузки с оператора.
...
В совокупности это формирует устойчивый административный контур: интерфейс показывает актуальную картину, а backend не допускает «зависших» состояний.
...
С точки зрения сопровождения полезным решением стало выделение повторяющихся служебных операций в отдельные функции и унификация обработчиков типовых сценариев. Это снижает дублирование, упрощает ревью изменений и делает кодовую базу более предсказуемой при развитии функциональности.

#### 2.2.6 Динамическое формирование слотов и защита от коллизий

Слоты формируются с учетом длительности услуги, обеденного окна и конца смены:
...
На этапе выдачи слотов дополнительно фильтруются уже занятые и заблокированные интервалы:
...
Этот двухэтапный подход уменьшает вероятность конфликтов и в интерфейсе, и в момент финальной записи.

#### 2.2.7 Обработка ошибок и пользовательская обратная связь

Для сценариев создания и редактирования автомобиля я добавил явную обработку конфликтов уникальности:
...
Такая схема дает пользователю предметное сообщение вместо общего «500 Internal Server Error», что уменьшает число повторных ошибочных действий.

#### 2.2.8 Ролевые ограничения на уровне представлений

Изоляция административного контура закреплена декораторами:
...
Это позволяет централизованно контролировать доступ к критичным операциям и не дублировать проверку прав в каждом фрагменте шаблонов.

#### 2.2.9 Практики производительности в прикладном коде

В пользовательском и административном интерфейсах активно используются выборки с предварительной загрузкой связей:
...
В аналитических блоках применяются агрегации на уровне БД:

```python
top_services_qs = (
    appt_qs.values("service_type__name")
    .annotate(c=Count("id"))
    .order_by("-c")[:5]
)
```

Это снижает объем Python-обработки и делает страницу стабильной при росте объема данных.

### 2.3 Реализация интеграций и прикладных сервисов

В этом разделе я рассматриваю интеграции как единый инженерный процесс: проектирование интерфейсов, контроль согласованности данных, снижение рисков повторных операций и обеспечение наблюдаемости выполнения. В проекте это реализовано через отдельные модули `payments`, `notifications` и `loyalty_program`, которые связаны с ядром системы, но не смешивают свою ответственность с веб-представлениями.
...
Такой подход устраняет дубли финансовых операций и снижает риск рассогласования между локальной БД и внешним провайдером. Также я использую двойную синхронизацию статуса: по пользовательскому действию и через webhook-события.
...
Эта проверка важна для корректной фиксации момента оплаты: поле `paid_at` не перезаписывается при повторной проверке того же платежа.
...
Дополнительно в сервисе уведомлений зафиксировано поведение при ошибках:
...
Это дает наблюдаемость в продакшене: ошибка доставки не «теряется», а попадает в логи с контекстом.
...
Связка модуля лояльности с оплатой сделана так, чтобы бонусные операции выполнялись только при корректных статусах платежа и не дублировались при повторной обработке событий.

Дополнительный защитный блок в сигналах:
...
За счет этого одно и то же событие webhook не приводит к повторному начислению бонусов.
...
С инженерной позиции это показывает, что сервисы связаны в единый бизнес-процесс с проверяемыми переходами состояния. Благодаря этому в систему можно добавлять новые интеграции (например, CRM или push-канал) без разрушения базового контура.

#### 2.3.5 Обработка webhook и безопасное завершение потока оплаты

Webhook-обработчик принимает событие `payment.succeeded`, ищет локальный платеж и запускает синхронизацию статуса:
...
Пользовательский маршрут проверки статуса дополняет webhook-схему:
...
Двойной канал (webhook + ручная проверка из интерфейса) полезен в реальных условиях нестабильной сети и отложенной доставки внешних событий.
...
Развертывание реализовано в двух режимах: локальный запуск для разработки и контейнерный запуск для стабильного окружения. В обоих режимах я использую конфигурацию через `.env`, чтобы отделить код от инфраструктурных параметров.
...
Контейнерный сценарий опирается на `docker-compose.yml` и включает `web`, `db`, `caddy`. В конфигурации зафиксированы `healthcheck`, политика перезапуска и постоянные тома данных.
...
db:
  ...
  healthcheck:
    test: ["CMD-SHELL", "pg_isready -U ${DB_USER:-autoservice_user} -d ${DB_NAME:-autoservice}"]
web:
  command: gunicorn autoservice_project.wsgi:application --bind 0.0.0.0:8000 --workers 3
```

Пайплайн поставки дополняется workflow для сборки Docker-артефакта и CI-проверками запуска приложения в Ubuntu/Windows.
...
С инженерной точки зрения важно, что в проекте предусмотрены механизмы эксплуатационной устойчивости: проверки доступности базы, политика перезапуска сервисов и автоматизированные CI-проверки контейнерной сборки/запуска.

Фрагмент CI-проверки запуска на Ubuntu:
...
Фрагмент CI-пайплайна сборки Docker-артефакта:

```yaml
- name: Build Docker image
  run: docker build -t $IMAGE_NAME:${{ steps.meta.outputs.version }} .

- name: Save image to tarball
  run: docker save $IMAGE_NAME:${{ steps.meta.outputs.version }} -o "$TARBALL"
```

Это уменьшает риск ручных регрессий при обновлениях и делает процесс поставки предсказуемым.
...
Пример проверки успешной аутентификации:
...
В этом примере используется тестовый пароль из набора автотестов. В рабочем окружении применяются политики сложных паролей, а учетные данные не хранятся в коде.

Пример проверки ограничения доступа в админ-контур:
...
Пример интеграционного кейса создания записи через форму:
...
Пример конкурентного сценария из `tests_performance.py`:
...
Пример проверки SLA по времени ответа:
...
Фактически критерий «готово к выпуску» у меня формируется как чек-лист:

- `python manage.py test` — успешно;
- ключевые пользовательские сценарии — без регрессий;
- статусные переходы `Appointment/Payment` — консистентны;
- performance-пороги тестов — соблюдены.
