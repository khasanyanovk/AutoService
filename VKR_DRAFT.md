...
REST API реализован на DRF и обеспечивает стандартный программный доступ к ключевым сущностям системы (автомобили, записи, справочники, административные ресурсы), а также JWT-аутентификацию. В проекте этот контур разрабатывался коллегой, поэтому в данной работе он фиксируется как важная интеграционная часть, обеспечивающая внешний и межклиентский доступ к функциональности.

Со стороны архитектуры для API важны единая маршрутизация через `DefaultRouter`, централизованные настройки безопасности и разделение пользовательских и административных endpoint-групп.

#### 1.5.2 Компоненты веб-клиента

Основной вклад автора сосредоточен в веб-клиенте, реализованном на server-side рендеринге Django templates. Этот контур включает проектирование пользовательских сценариев, построение административных интерфейсов и согласование UI-логики с backend-ограничениями. Технически реализация опирается на Django view-функции, серверные формы и встроенную валидацию, поэтому операции в интерфейсе напрямую соответствуют доменным правилам системы.

Пользовательский контур охватывает регистрацию и вход, профиль, управление автомобилями, запись на обслуживание, просмотр и отмену записей, а также работу с динамическими формами выбора филиала, услуги и доступного времени.

Административный контур реализует ежедневные операционные задачи: дашборд, управление филиалами и услугами, ведение пользователей и записей, обработку отзывов, а также контроль расписания через блокировку слотов. Отдельный акцент сделан на ролевом разграничении действий, чтобы административные операции были изолированы от клиентского контура и выполнялись только в допустимых сценариях.
...
Практический результат авторской реализации веб-части — целостный интерфейс работы для клиента и администратора, в котором ключевые бизнес-процессы (запись, изменение статусов, управление сущностями, коммуникация с пользователем) выполняются через единый серверный контур `views + forms + validation` без расхождения с backend-правилами.
